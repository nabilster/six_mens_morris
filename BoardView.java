...
public class BoardView extends JPanel {
    ...
    public void highlightNode (Node node){  // repaint new node behind to highlight
        highlight[0]=node.getRelX();
        highlight[1]=node.getRelY();
        repaint();
    }

    public void clearHighlight(){ // remove highlight
        highlight[1]=nullVal;
        repaint();
    }
    ...
    //removing token
    public void removeToken(double oldX, double oldY){
        int i=0;
        while (i<tokens.size()){
            if (tokens.get(i)[1]==oldX&&tokens.get(i)[2]==oldY){
                tokens.get(i)[1]=nullVal; tokens.get(i)[2]=nullVal;
                repaint();
                break;
            }
            i++;
    ...
    public void moveToken (double oldX, double oldY, double newX, double newY){ //moving token from one place to another
        int i=0;
        while (i<tokens.size()){
            if (tokens.get(i)[1]==oldX&&tokens.get(i)[2]==oldY){
